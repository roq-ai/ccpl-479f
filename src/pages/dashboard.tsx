...
import { getTestimonials } from 'apiSdk/testimonials';
import { TestimonialInterface } from 'interfaces/testimonial';
import { getContactForms } from 'apiSdk/contact-forms';
import { ContactFormInterface } from 'interfaces/contact-form';
import { getProductImages } from 'apiSdk/product-images';
import { ProductImageInterface } from 'interfaces/product-image';

import { ProductDetailsListPage as ShowCaseList } from 'pages/product-details';

function DashboardPage() {
  const { session } = useSession();

  const testimonialStat = useSWR<PaginatedInterface<TestimonialInterface>>(
    () => `/testimonials`,
    () => getTestimonials({ limit: 1, offset: 0 }),
  );

  const contactFormStat = useSWR<PaginatedInterface<ContactFormInterface>>(
    () => `/contact-forms`,
    () => getContactForms({ limit: 1, offset: 0 }),
  );

  const productImageStat = useSWR<PaginatedInterface<ProductImageInterface>>(
    () => `/product-images`,
    () => getProductImages({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Testimonials',
      result: testimonialStat?.data?.totalCount,
      isLoading: testimonialStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Contact Forms',
      result: contactFormStat?.data?.totalCount,
      isLoading: contactFormStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Product Images',
      result: productImageStat?.data?.totalCount,
      isLoading: productImageStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`product-details`} passHref legacyBehavior>
                  <Button
