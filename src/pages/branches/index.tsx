...
import { getBranches, deleteBranchById } from 'apiSdk/branches';
import { BranchInterface } from 'interfaces/branch';

type ColumnType = ColumnDef<BranchInterface, unknown>;
...
export function BranchListPage(props: BranchListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getBranches({
        ...
        order: params.order,
        searchTermKeys: ['address.contains', 'brnach_head.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BranchInterface>>(
    () => `/branches?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BranchInterface) => {
    if (hasAccess('branch', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/branches/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'address', header: 'Address', accessorKey: 'address' },
    { id: 'brnach_head', header: 'Brnach Head', accessorKey: 'brnach_head' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/branches/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('branch', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/branches/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('branch', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="branch" />
        </Flex>

        {hasAccess('branch', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/branches/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
